...
from models import db, User, Planet, Character, Vehicle, Favorite, Favorite_Character, Favorite_Planet, Favorite_Vehicle
...
def add_favorite_planet(planet_id):
    ...
    favorite = Favorite_Planet(user_id=user_id, planet_id=planet_id)
...
def delete_favorite_planet(planet_id):
    single_planet = Favorite_Planet.query.get(planet_id)
    if single_planet is None:
...
def add_favorite_character(character_id):
    ...
    if 'user_id' not in body:
        raise APIException('You must send the user id', status_code=400)
    user_id = body['user_id']
    favorite = Favorite_Character(user_id=user_id, character_id=character_id)
    db.session.add(favorite)
    db.session.commit()
...
def delete_favorite_character(character_id):
    single_character = Favorite_Character.query.get(character_id)
...
def add_favorite_vehicle(vehicle_id):
    ...
    if 'user_id' not in body:
        raise APIException('You must send the user id', status_code=400)
    user_id = body['user_id']
    favorite = Favorite_Vehicle(user_id=user_id, vehicle_id=vehicle_id)
    db.session.add(favorite)
    db.session.commit()
    return jsonify({'message': 'Favorite character added successfully'})
...
def delete_favorite_vehicle(vehicle_id):
    single_vehicle = Favorite_Vehicle.query.get(vehicle_id)
    if single_vehicle is None:
        raise APIException("The vehicle doesn't exist", status_code=400)
    db.session.delete(single_vehicle)
    db.session.commit()
    return jsonify({'msg': 'Completed'})
