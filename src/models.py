...
class Favorite_Character(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), unique=True, nullable=False)
    user = db.relationship(User)
    character_id = db.Column(db.Integer, db.ForeignKey('favorite.character_id'), nullable=True)
    character = db.relationship(Favorite)
    
    def __repr__(self):
        return f"The user with id {self.user_id} has favorited character with id {self.character_id}"
...
class Favorite_Planet(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), unique=True, nullable=False)
    user = db.relationship(User)
    planet_id = db.Column(db.Integer, db.ForeignKey('favorite.planet_id'), nullable=True)
    planet = db.relationship(Favorite)
    
    def __repr__(self):
        return f"The user with id {self.user_id} has favorited planet with id {self.planet_id}"
...
class Favorite_Vehicle(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), unique=True, nullable=False)
    user = db.relationship(User)
    vehicle_id = db.Column(db.Integer, db.ForeignKey('favorite.vehicle_id'), nullable=True)
    vehicle = db.relationship(Favorite)
    
    def __repr__(self):
        return f"The user with id {self.user_id} has favorited vehicle with id {self.vehicle_id}"
